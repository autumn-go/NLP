...
node_s = set()
edge_s = set()
prescription_name_s = set()
# namefile = "name.txt"
# file1 = "file1.txt"
# file1_out = "file1_out.txt"

# prescription_name_s = read_file_into_set(namefile)
# print(len(prescription_name_s))
# exit()
cnt_get_name = 0
	...
	for file in files:
		...
		with open(jsonfile, 'r') as indata:
			...
			for key,rcd in json_dict.items():
				...
				# flag = 0
				# if prescription_name in prescription_name_s:
				# 	flag = 1
				# 	cnt_get_name += 1
				# 	read_file1_part(file1, prescription_name, file1_out)
				...
				for item in medicine_arr:
					...
					# 额外收集方剂和草药对应信息
					# if flag:
					# 	rel_str = "%s %s" % (prescription_name, herb)
					# 	write_into_file(file1_out, rel_str)


				#break
				cnt += 1
				count += 1
			print("get records: %d" % cnt)
	
			#log this file
			#write_into_file(log, "%s %s" % (file, cnt))
print("Total records: %d" % count)
print("Total name found: %d" % cnt_get_name)
fd.close()
fd_node.close()
fd_edge.close()
